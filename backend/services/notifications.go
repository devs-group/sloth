...
func (s *S) CreateNotification(payload Notification, tx *sqlx.Tx) error {
	query := `
		INSERT INTO notifications (subject, content, notification_type, recipient_id) 
		VALUES ($1, $2, $3, $4);
	`
	_, err := tx.Exec(query, payload.Subject, payload.Content, payload.RecipientID, payload.NotificationType)
	if err != nil {
