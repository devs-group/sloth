                ...
                <div class="flex flex-col gap-2" v-if="hasServices">
                    ...
                    <div class="flex gap-6">
                        <div
                            ...
                            v-for="(service, _) in Object.values(
                                project.services,
                            )"
                        >
                            <ServiceDetail
...
onMounted(() => {
    fetchProject(projectID)
        .then(async (fetchedProject) => {
            ...
            try {
                const records = await fetchServiceStates(
                ...
                if (records) {
                    Object.keys(records).forEach((key) => {
                        ...
                        console.log(
                            `Service ID: ${key}, State: ${state.state}, Status: ${state.status}`,
                        );
                    });
                }
            } catch (error) {
                console.error(
                    "Failed to fetch states for services in the project",
                    project.value?.id,
                    error,
                );
            }
...
const addNewService = (service: ServiceSchema) => {
    const s = addService(service);
    if (project.value && s?.name !== "") {
