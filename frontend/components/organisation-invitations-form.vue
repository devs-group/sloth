...
<div v-if="props.invitations && props.invitations.length > 0">
    <template v-for="invitation in props.invitations" :key="invitation.user_id">
        <OrganisationInvitationRow :invitation="invitation" :organisation_id="props.organisation.id"  @on-withdraw="props.emits.withdrawInvitation()"></OrganisationInvitationRow>
    </template>
    </div>
