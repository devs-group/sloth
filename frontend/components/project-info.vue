<template>
  <div class="flex flex-col gap-2">
    ...
    <div class="flex flex-col gap-1 max-w-full">
      <p class="text-sm text-prime-secondary-text">Unique project name</p>
    ...
    <div
      v-if="project.services && project.services.find((s) => s.public.enabled)"
      class="flex flex-col gap-1 max-w-full"
    >
      <p class="text-sm text-prime-secondary-text">Public URLs</p>
      <div
        v-for="service in project.services.filter((s) => s.public.enabled)"
        :key="service.usn"
      >
        <template
          v-if="service.public.hosts.some((url) => url.trim().length > 0)"
        >
          <div
