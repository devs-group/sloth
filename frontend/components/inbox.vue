<template>
  ...
  >
    ...
    <Badge
      ...
      :value="notifications.length > 9 ? '9+' : notifications.length"
      class="absolute top-0 right-0 bg-red-600 text-white"
    />
  </Button>
      ...
      <template v-if="notifications.length > 0">
        ...
        >
          <div class="flex flex-row gap-2">
            <Avatar
              :icon="notification.notification_type === NotificationType.INVITATION ? 'pi pi-envelope' : 'pi pi-info-circle'"
              class="mr-2"
              shape="circle"
            />
            <div>
