<template>
  <div class="flex flex-col flex-1 gap-2">
    ...
    <div class="flex gap-12 overflow-auto flex-1">
      ...
      >
        <ProgressSpinner v-if="isLoadingServiceStates" />
        <ServiceDetail
          v-else
            ...
            >
              <InputGroup>
                <InputText
                  ...
                  @change="validate(sIdx, 'ports', pIdx)"
                />
                <IconButton
        ...
        <div class="flex flex-col gap-1">
          ...
          <div class="flex flex-col gap-2">
            <div>
              <Button
                label="Add Persistent Volume"
                severity="secondary"
                icon="pi pi-plus"
                @click="onAddPersistentVolume"
              />
            </div>
            <div
              v-for="(volume, vIdx) in service.volumes"
              :key="`volume-${sIdx}-${vIdx}`"
              class="flex flex-col"
            >
              <InputGroup>
                <InputText
                  v-model="service.volumes[vIdx]"
                  @change="validate(sIdx, 'volumes', vIdx)"
                />
                <IconButton
            ...
            >
              <InputGroup>
                <InputText
                  v-model="env[0]"
                  placeholder="Key"
                  @change="validate(sIdx, 'env_vars', eIdx, 0)"
                />
                <InputText
                  v-model="env[1]"
                  placeholder="Value"
                  @change="validate(sIdx, 'env_vars', eIdx, 1)"
                />
                <IconButton
...
const onAddPersistentVolume = () => {
  dialog.open(PersistentVolumeDialog, {
    props: {
      header: `Create Persistent Volume for ${props.project.name}`,
      ...ModalConfig,
    },
    data: {
      project: props.project,
